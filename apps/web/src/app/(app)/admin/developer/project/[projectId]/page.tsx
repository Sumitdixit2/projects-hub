...
export default function DeveloperProjectDetailsPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        toast.error(error.message || "Failed to fetch assigned project details");
      } finally {
        setLoading(false);
      }
    };
      ...
      cell: (m: any) => (
        <div className="flex justify-end">
          <Button
            variant="outline"
            size="sm"
            onClick={() => router.push(`/admin/developer/milestone/${m.id}`)}
      ...
      <DashboardLayout
        ...
        subtitle={
          <span className="flex items-center gap-2 text-muted-foreground text-[12px] font-mono">
            ...
            <button
              onClick={() => router.push("/admin/developer/projects")}
              className="hover:text-foreground transition-colors flex items-center gap-1"
            >
              <ArrowLeft className="w-3 h-3" /> Execution Queue
            </button>
          </span>
        }
      >
        <div className="space-y-8 mt-4">
          ...
          {milestones.length > 0 && (
            <div className="space-y-2">
              <div className="flex items-center justify-between text-[11px] font-mono text-muted-foreground">
                <span className="uppercase tracking-widest">Delivery Pipeline</span>
              ...
              <p className="text-[10px] text-muted-foreground/60 font-mono">
                {milestonesCompleted} / {milestones.length} execution steps resolved
              </p>
            </div>
          )}
          ...
          <Card className="p-5 bg-[#050505] border-l-2 border-l-primary/30 border-y-border border-r-border">
            ...
            <p className="text-[13px] text-foreground/80 leading-relaxed whitespace-pre-wrap">
              {project.description || "No operational scope defined."}
            </p>
          </Card>
          ...
          <div className="space-y-3">
            ...
            <DataLedgerTable
              ...
              keyExtractor={(m) => m.id}
              onRowClick={(m) => router.push(`/admin/developer/milestone/${m.id}`)}
              emptyStateMessage="No workload provisioned for this project."
            />
          </div>
