...
export default function StaffMilestoneDetailsPage() {
    ...
    return (
      <AppShell role="admin">
        <div className="flex h-[80vh] flex-col items-center justify-center text-center">
          <p className="text-[13px] font-mono text-destructive uppercase tracking-widest mb-2">MILESTONE_ACCESS_DENIED</p>
          <p className="text-[12px] text-muted-foreground opacity-70">You are not authorized to access this milestone space.</p>
          <Button variant="outline" size="sm" className="mt-6" onClick={() => router.back()}>
            <ArrowLeft className="w-3.5 h-3.5 mr-2" /> Return
          </Button>
        </div>
      </AppShell>
    );
  ...
  if (!milestone || errorStatus === 404) {
    return (
      <AppShell role="admin">
        <div className="flex h-[80vh] flex-col items-center justify-center text-center">
          <p className="text-[13px] font-mono text-muted-foreground uppercase tracking-widest mb-2">MILESTONE_NOT_FOUND</p>
          <p className="text-[12px] text-muted-foreground opacity-70">The requested milestone ID could not be located in the system.</p>
          <Button variant="outline" size="sm" className="mt-6" onClick={() => router.back()}>
            <ArrowLeft className="w-3.5 h-3.5 mr-2" /> Return
          </Button>
        </div>
      ...
      <DashboardLayout
        ...
        subtitle={
          <span className="flex items-center gap-1.5 text-muted-foreground text-[12px] font-mono">
            ...
            <button
              onClick={() => router.push(`/admin/staff/project/${milestone.project_id}`)}
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {isOverdue && (
              <div className="flex items-start gap-2.5 p-3 rounded-md border border-red-500/20 bg-red-500/5">
                ...
                <div>
                  <p className="text-[12px] font-medium text-red-400">Milestone Overdue</p>
                  <p className="text-[11px] text-muted-foreground mt-0.5">
                    This milestone passed its target date on {dueDate ? formatDate(dueDate) : "—"}.
                  </p>
                </div>
              </div>
            )}
            ...
            <Card className="p-5 bg-[#050505] border-l-2 border-l-primary/30 border-y-border border-r-border">
              ...
              <p className="text-[13px] text-foreground/80 leading-relaxed whitespace-pre-wrap">
                {milestone.description || "No operational scope defined for this milestone."}
              </p>
            </Card>
          ...
          <div className="space-y-5">
            ...
            <Card className="p-5 bg-[#050505] border-border">
              ...
              <div className="mt-4 flex items-start gap-2 p-3 rounded-md border border-border/50 bg-black/40">
                ...
                <p className="text-[11px] text-muted-foreground leading-relaxed">
                  Status changes are propagated in realtime. Clients receive notifications on major transitions.
                </p>
              </div>
            </Card>
            ...
            <Card className="p-5 bg-[#050505] border-border">
              <div className="flex items-center gap-2 mb-3 text-muted-foreground">
                ...
                <h3 className="text-[10px] font-mono uppercase tracking-widest">Project Context</h3>
              </div>
              <Button
                variant="outline"
                size="sm"
