      ...
      cell: (item: projectType) => (
        <div className="flex justify-end">
          <Button 
            variant="outline" 
            size="sm" 
            onClick={() => router.push(`/admin/staff/project/${item.id}`)}
  ...
  return (
    <AppShell role="admin">
      <DashboardLayout 
        ...
        subtitle="Agency-wide initiative tracking and execution."
        actions={
          <Button onClick={() => router.push('/admin/staff/createProject')} size="sm" className="gap-2">
      ...
      >
        {loading ? (
          ...
          <div className="py-12 text-center bg-black border border-border mt-4 rounded-xl">
            <p className="text-[12px] font-mono text-destructive uppercase tracking-widest">ACCESS_RESTRICTED</p>
            <p className="text-[11px] text-muted-foreground mt-2 opacity-60">You do not have authorization to view the agency project registry.</p>
          </div>
        ) : (
          <>
            <MetricStrip 
            ...
            <div className="mt-4">
              <DataLedgerTable 
                ...
                emptyStateMessage="NO_PROJECTS_FOUND"
              />
            </div>
          </>
        )}
